...
const CityMap = dynamic(() => import("@/components/CityMap"), {
  ssr: false,
  loading: () => <div className="w-full h-full flex items-center justify-center bg-neutral-100"><span className="text-neutral-500 text-sm">Chargement…</span></div>,
})
...
export default async function CityPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const shareBase = proj2050 != null
    ? `À ${city.name}, le GIEC (CMIP6) projette +${Math.abs(proj2050).toFixed(1)}°C d'ici 2050. `
    : `À ${city.name} (${city.region}), combien ça va chauffer d'ici 2050 ? `

  const shareUrls = {
    x: `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareBase + cityUrl)}`,
    linkedin: `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(cityUrl)}`,
    whatsapp: `https://api.whatsapp.com/send?text=${encodeURIComponent(`🌡️ ${shareBase}${cityUrl}`)}`,
    email: `mailto:?subject=${encodeURIComponent(`La chaleur à ${city.name} · cestchaud.fr`)}&body=${encodeURIComponent(`${shareBase}\n\n${cityUrl}`)}`,
  }
